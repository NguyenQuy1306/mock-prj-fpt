...
// import com.cnpm.lms.service.CourseService;
...
public class InstructorController {
    ...
    public ResponseEntity<ApiResponse<InstructorResponse>> updateInstructor(@PathVariable Long id,
            @ModelAttribute @Valid @RequestBody InstructorUpdateRequest instructorUpdateRequest,
    ...
    // public ResponseEntity<ApiResponse<Page<CourseDetailResponse2>>>
    // getCourse(@PathVariable Long id,
    // @RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "5")
    // int size) {
    // try {
    // Pageable pageable = PageRequest.of(page, size);
    // Page<CourseDetailResponse2> coursePage =
    // courseService.getCoursebyInstructorId(id, pageable);
    // MetadataResponse metadata = new MetadataResponse(
    // coursePage.getTotalElements(),
    // coursePage.getTotalPages(),
    // coursePage.getNumber(),
    // coursePage.getSize(),
    // (coursePage.hasNext() ? "/api/instructors/" + id + "/course?page=" +
    // (coursePage.getNumber() + 1)
    // : null),
    // (coursePage.hasPrevious()
    // ? "/api/instructors/" + id + "/course?page=" + (coursePage.getNumber() - 1)
    // : null),
    // "/api/instructors/" + id + "/course?page=" + (coursePage.getTotalPages() -
    // 1),
