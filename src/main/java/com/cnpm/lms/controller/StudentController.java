...
import com.cnpm.lms.service.impl.UserDetailsServiceImpl;
...
public class StudentController {
    ...
    public ResponseEntity<ApiResponse<List<StudentResponse>>> getAllStudents(@RequestParam(defaultValue = "0") int page,
    ...
    // updateStudentAddress(@PathVariable Long id,
    // @RequestBody @Valid UserAddressRequest studentAddressRequest, BindingResult
    // bindingResult) {
    // try {
    // if (bindingResult.hasErrors())
    // throw new Exception("Request invalid");
    // UserAddressResponse studentResponse = studentService.updateStudentAddress(id,
    ...
    // try {
    // Pageable pageable = PageRequest.of(page, size);
    // Page<EnrollmentResponse> enrollmentPage =
    // studentService.getCoursesByStudentId(id, pageable);
    ...
    // (enrollmentPage.hasNext() ? baseUrlStr + (enrollmentPage.getNumber() + 1) +
    // "&size=" + size : null),
    // (enrollmentPage.hasPrevious() ? baseUrlStr + (enrollmentPage.getNumber() - 1)
    // + "&size=" + size
    // : null),
    // baseUrlStr + "page=" + (enrollmentPage.getTotalPages() - 1) + "&size=" +
    // size,
    // baseUrlStr + "page=0&size=" + size);
    ...
    // addStudentToCourse(@PathVariable Long studentId,
    ...
    // @PreAuthorize("hasRole('ROLE_ADMIN') or (hasRole('ROLE_STUDENT') and
    // authentication.principal.getId() == #studentId)")
    // @GetMapping("/{studentId}/statistic")
    // public ResponseEntity<ApiResponse<StudentStatisticResponse>>
    // studentStatistic(@PathVariable Long studentId) {
    // try {
    ...
    // ResponseEntity<ApiResponse<SectionCompleteResponse>>
    // getCurrentSection(@PathVariable Long studentId,
    // @PathVariable Long courseId) {
    ...
    // @PreAuthorize("hasRole('ROLE_ADMIN') or (hasRole('ROLE_STUDENT') and
    // authentication.principal.getId() == #request.studentId)")
    // @PutMapping("/complete-section")
    // ResponseEntity<ApiResponse<SectionCompleteResponse>> completeCurrentSection(
    // @Valid @RequestBody SectionCompleteRequest request) {
    // ApiResponse<SectionCompleteResponse> apiResponse = new ApiResponse<>();
