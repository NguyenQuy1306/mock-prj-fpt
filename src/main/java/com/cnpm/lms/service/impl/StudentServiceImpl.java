...
import com.cnpm.lms.constants.ContentType;
import com.cnpm.lms.exception.ApplicationException;
import com.cnpm.lms.exception.DuplicatePhoneNumberException;
import com.cnpm.lms.exception.InvalidFileTypeException;
import com.cnpm.lms.exception.NotFoundException;
import com.cnpm.lms.exception.ValidationException;
import com.cnpm.lms.model.entity.*;
// import com.cnpm.lms.model.mapper.CourseMapper;
import com.cnpm.lms.model.mapper.UserMapper;
// import com.cnpm.lms.model.request.SectionCompleteRequest;
import com.cnpm.lms.model.request.StudentRequest;
import com.cnpm.lms.model.request.UserAddressRequest;
import com.cnpm.lms.model.response.*;
import com.cnpm.lms.repository.*;
// import com.cnpm.lms.service.CartService;
// import com.cnpm.lms.service.CloudinaryService;
import com.cnpm.lms.service.StudentService;
// import com.cnpm.lms.util.FileAsyncUtil;
...
public class StudentServiceImpl implements StudentService {
    ...
    // @Override
    // @Transactional
    // public Page<EnrollmentResponse> getCoursesByStudentId(Long studentId,
    // Pageable pageable) {
    // try {
    ...
    // enrollmentRepository.findByStudent_UserId(studentId, pageable);
    // Page<EnrollmentResponse> enrollmentResponses = enrollments.map(enrollment ->
    ...
    // cartItemsRepository.findAllByCart_CartId(cart.getCartId());
    // List<CourseResponse> courseResponses = cartItems.stream()
    // .map(cartItem ->
    // courseMapper.toResponse(cartItem.getCourse())).collect(Collectors.toList());
    ...
    // @Override
    // @Transactional
    // public Page<CourseResponseForCart> getListCourseFromCart(Long studentId,
    // Pageable pageable) {
    // try {
    // Student student = studentRepository.findById(studentId).orElse(null);
    // if (student == null) {
    ...
    // cartItemsRepository.findAllByCart_CartId(cart.getCartId(), pageable);
    ...
    // try {
    // EnrollmentResponse enrollmentResponse = addStudentToCourse(studentId,
    // courseId);
    ...
    // @Override
    // public HashMap<String, Integer> getCoursesPurchasedLastFiveYears(Long
    // studentId) {
    ...
    // enrollment.getEnrollmentDate().toInstant().atZone(ZoneId.systemDefault())
    // .toLocalDate();
    // if ((enrollmentDate.isAfter(startOfYear) ||
    // enrollmentDate.isEqual(startOfYear)) &&
    ...
    // enrollmentRepository.findByStudent_UserId(studentId);
    ...
    // HashMap<String, Integer> finishCourseFiveYears = new HashMap<String,
    // Integer>();
    ...
    // StudentStatisticResponse statisticResponse = new
    // StudentStatisticResponse(getTotalPurchaseCourse(studentId),
    ...
    // public UserAddressResponse updateStudentAddress(Long userId,
    // UserAddressRequest addressRequest) {
    // try {
    // Student user = studentRepository.findById(userId)
    // .orElseThrow(() -> new ApplicationException("Student not found with id: " +
    // userId));
    ...
    // catch (com.cnpm.lms.exception.NotFoundException ex) {
    // throw ex;
    ...
    // @Override
    // public SectionCompleteResponse completeSection(SectionCompleteRequest
    // request) {
    // SectionCompleteResponse response = new SectionCompleteResponse();

    // Section currentSectionInRequest =
    ...
    // enrollmentRepository.findByStudent_UserIdAndCourse_CourseId(request.getStudentId(),
    // course.getCourseId());
    // if (enrollment == null)
    ...
    // .findByCourse_CourseIdAndPosition(course.getCourseId(),
    // enrollment.getCurrentSectionPosition())
    // .orElseThrow(() -> new NotFoundException("No more sections to complete"));

    // // mismatch between current section in request and current section in
    // database
    // if (currentSectionInRequest.getSectionId() !=
    // currentSectionInDB.getSectionId()) {
    ...
    // enrollment.setCurrentSectionPosition(enrollment.getCurrentSectionPosition() +
    // 1);
    // }
    ...
    // @Override
    // public SectionCompleteResponse getCurrentSection(Long studentId, Long
    // courseId) {
    // SectionCompleteResponse response = new SectionCompleteResponse();
    ...
    // .findByCourse_CourseIdAndPosition(courseId,
    // enrollment.getCurrentSectionPosition())
